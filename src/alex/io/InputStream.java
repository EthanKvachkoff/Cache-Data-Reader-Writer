...
public final class InputStream extends Stream {
	...
	public void writeString(String value) {
		byte[] bytes = value.getBytes(); // Convert the string to bytes
		for (byte b : bytes) {
			writeInt(b); // Write each byte
		}
		writeInt(0); // Write the null terminator
	}

	public void replaceStringInStream(String newName) {
		// Reset the offset to the start of the string (you may need to adjust based on how offsets are handled)
		int startOffset = offset - (readString().length() + 1); // Assuming the original string length is correct
		setOffset(startOffset);
	
		// Now, write the new string into the buffer
		for (char c : newName.toCharArray()) {
			writeByte((byte) c);  // Ensure that you're writing each byte properly
		}
	
		// Add a null terminator (0) at the end of the string
		writeByte((byte) 0);
	
		// Update the offset to reflect the new string length
		offset += newName.length() + 1;  // Including null terminator
	...
	public void writeByte(byte value) {
		checkCapacity(1); // Ensure the buffer has enough space
		buffer[offset++] = value;
	}
